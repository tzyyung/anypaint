...
extension String {
	...
	本機 vendored patch：原本用 `Bundle.module`（SwiftPM 對 executable target 生成的
	accessor 只認 .app 根與開發者絕對 .build 路徑，兩者皆無時 fatalError）。
	改為從 app 的 Contents/Resources 找資源 bundle，找不到退回 Bundle.main
	（NSLocalizedString 對缺失 key 回傳 key 本身、絕不崩）。
	*/
	var localized: String {
		let name = "KeyboardShortcuts_KeyboardShortcuts"
		let bundle = Bundle.main.url(forResource: name, withExtension: "bundle")
