...
/// 設定視窗：全域快鍵、框選看門狗、輸出（檔名樣板／快速儲存／自動儲存／通知）。
public final class SettingsWindowController: NSWindowController {
    ...
    public init() {
        let window = NSWindow(
            contentRect: NSRect(x: 0, y: 0, width: 480, height: 380), // 初始值；實際高度由 buildUI 依內容實算
    ...
    private func buildUI() {
        ...
        let outputHeading = NSTextField(labelWithString: "輸出")
        outputHeading.font = .boldSystemFont(ofSize: 13)
        ...
        // 視窗高度依內容實算（別手猜 magic number——先前審查抓過 380 不夠）。
        stack.layoutSubtreeIfNeeded()
        window?.setContentSize(NSSize(width: 480, height: stack.fittingSize.height + 40))
        window?.center()   // resize 是頂邊固定向下長，init 時的 center 已偏——重新置中
    }
