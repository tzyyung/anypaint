...
- 動畫截圖：框選區域錄成動畫（預設 `⌘⇧R`），可設錄製秒數、顯示游標與點擊高亮圈（設定可關）、
  GIF fps（8/10/12/15/20 五檔，預設 12）與 MP4 是否用 HEVC（檔案較小）；完成後預覽視窗可剪裁時間段、拍快照、
  直接拖曳出 MP4，並匯出 GIF（偵測到 gifski 會自動用它產生更高品質）、全彩 APNG，
  或（偵測到 img2webp 才出現）WebP。不錄音訊。
